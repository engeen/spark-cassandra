...
$test_hosts = {
  cassandra:      { ip: "192.168.5.3", mem: 2048 , roles: ["cassandra"], hostname: "cassandra", ports: {} },
  spark:      { ip: "192.168.5.4", mem: 2048, roles: ["spark"], hostname: "spark", ports: { 10000 => 10000 } }
...
Vagrant.configure("2") do |config|
  ...
  $test_hosts.keys.each do |host|
    config.vm.define host.to_s, autostart: false do |srv|
      ...
      srv.librarian_chef.cheffile_dir = "chef"
      srv.vm.provision :chef_solo do |chef|
        ...
        $test_hosts[host][:roles].each do |role|
          chef.add_role(role)
        end

        chef.node_name = $test_hosts[host][:hostname]
